...
resource "azurerm_subnet" "aks" {
  ...
  enforce_private_link_endpoint_network_policies = true
  enforce_private_link_service_network_policies  = true
  service_endpoints                              = ["Microsoft.ServiceBus","Microsoft.Storage","Microsoft.KeyVault"]
...
resource "azurerm_virtual_network_peering" "vn_default_peer" { #Peering to remote vnet
  name                      = "default-to-vnet-vpnhub-${var.env}"         #set up suffix ${var.env} at end
  resource_group_name       = var.resource_group
  virtual_network_name      = azurerm_virtual_network.vnet.name
...
resource "azurerm_virtual_network_peering" "vnet_hub_peer" { #Here, we peer back, from shared vnet to our main vnet.
  ...
  name                      = "vnet-vpnhub-to-defualt-${var.env}" #set up suffix ${var.env} at end 

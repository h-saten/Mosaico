...
@Component({
  // selector: 'app-article-cover-upload',
  templateUrl: './article-cover-upload.component.html',
  styleUrls: ['./article-cover-upload.component.scss']
})
export class ArticleCoverUploadComponent implements OnInit, OnDestroy {
  @Input() blankImgUrl: string;
  ...
  constructor(public modalService: NgbActiveModal, private toastr: ToastrService, private translateService: TranslateService, private tokenPageService: TokenPageService,
  ...
  submit(): void {
    ...
    if (logo) {
      this.isLoading = true;
      this.subs.sink = this.tokenPageService.updateArticleCover(logo.data, this.projectId).subscribe((res) => {
        if (res) {
          this.subs.sink = this.translateService.get('MODALS.PROJECT_ARTICLE_COVER_UPLOAD.MESSAGES.SUCCESS').subscribe((t) => this.toastr.success(t));
      ...
      this.translateService.get('MODALS.PROJECT_ARTICLE_COVER_UPLOAD.MESSAGES.INVALID_FORM').subscribe((t) => {
        this.toastr.error(t);
      });
    }
  }

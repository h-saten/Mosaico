...
import { BlockchainNetworkType, ErrorHandlingService, FormBase } from "mosaico-base";
import {
  CCQuoteResponse,
  InitiateKangaTransactionResponse,
  KangaPaymentCurrency,
  KangaPaymentCurrencyEstimate,
  OrdersService,
...
import { KangaPostPurchaseInfoComponent } from './kanga-post-purchase-info/kanga-post-purchase-info.component';
import { MosaicoPaymentService } from '../payment.service';

interface KangaPaymentCurrencyOption {
  displayName: string;
  type: KangaPaymentCurrency;
  isCryptoCurrency: boolean;
}

@Component({
  selector: 'app-kanga-exchange',
  templateUrl: './kanga-exchange.component.html',
  styleUrls: ['./kanga-exchange.component.scss']
})
export class KangaExchangeComponent extends FormBase implements OnInit, OnChanges, OnDestroy {
  ...
  @ViewChild('kangaPostPurchaseModal') kangaPostPurchaseModal: KangaPostPurchaseInfoComponent;
  ...
  initiatedTransaction: InitiateKangaTransactionResponse;
  kangaRedirectUrl: string;
  ...
  constructor(
    ...
    super();
    this.createForm();
    this.paymentService = new MosaicoPaymentService('KANGA_EXCHANGE', ordersService, userService);
  }
  ...
  private getKangaPaymentDetails(): void {
    if (this.showPhoneVerification === false) {
      let refCode = this.form.get('refCode')?.value;
      if(!refCode || refCode.length === 0){
        refCode = this.refCode;
      }
      this.subs.sink = this.transactionService.initKangaTransaction({
        paymentCurrency: this.getKangaCurrency(this.paymentService.currency),
        tokenAmount: this.paymentService.tokenAmount,
        projectId: this.projectSlug,
        currencyAmount: this.paymentService.paymentAmount,
        refCode
      }).subscribe((res) => {
        this.initiatedTransaction = res.data;
        this.toastr.success('Transaction processing in progress.');
        this.kangaRedirectUrl = res.data.redirectUrl;
        this.kangaPostPurchaseModal?.open(this.kangaRedirectUrl);
        this.subs.sink = this.kangaPostPurchaseModal.closed.subscribe((res) => {
          if(res === true) {
  ...
  private getKangaCurrency(currency: string): KangaPaymentCurrency {
    switch(currency) {
      case "PLN":
        return "oPLN";
      case "USD":

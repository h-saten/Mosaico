...
export class LayoutService {
  private layoutConfigLocalStorageKey: string;
  ...
  constructor(private configService: ConfigService) {
    this.layoutConfigLocalStorageKey = `${configService.getConfig().appVersion}-layoutConfig`
  }

  initConfig(): void {
    const configFromLocalStorage = localStorage.getItem(
      this.layoutConfigLocalStorageKey
    );
  ...
  private removeConfig(): void {
    localStorage.removeItem(this.layoutConfigLocalStorageKey);
  }

...
{
    public class RabbitMQMessageAggregator : IConsumer<CloudEvent>
    {
        private readonly IEnumerable<IEventHandler> _eventHandlers;
        private readonly ILogger _logger;

        public RabbitMQMessageAggregator(IEnumerable<IEventHandler> eventHandlers = null, ILogger logger = null)
        {
            _logger = logger;
            _eventHandlers = eventHandlers;
        }

        public virtual async Task Consume(ConsumeContext<CloudEvent> context)
        {
                ...
                if (attribute == null || (attribute?.Type != null && context.Message.Type == attribute.Type.FullName))
                {
                    ...
                    try
                    {
                        await eventHandler.HandleAsync(context.Message).ContinueWith(async (task) =>
                        {
                            if (task.IsCompleted)
                            {
                                await LogSuccessAsync(eventHandler.GetType().FullName,

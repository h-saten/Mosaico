...
using Mosaico.Integration.Blockchain.Ethereum.CrowdsaleBase.ContractDefinition;

namespace Mosaico.Integration.Blockchain.Ethereum.CrowdsaleBase
{
    public partial class CrowdsaleBaseService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CrowdsaleBaseDeployment crowdsaleBaseDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CrowdsaleBaseDeployment>().SendRequestAndWaitForReceiptAsync(crowdsaleBaseDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CrowdsaleBaseDeployment crowdsaleBaseDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CrowdsaleBaseDeployment>().SendRequestAsync(crowdsaleBaseDeployment);
        }

        public static async Task<CrowdsaleBaseService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CrowdsaleBaseDeployment crowdsaleBaseDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, crowdsaleBaseDeployment, cancellationTokenSource);
            return new CrowdsaleBaseService(web3, receipt.ContractAddress);
        }
        ...
        public CrowdsaleBaseService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
        ...
        public Task<string> RefundRequestAsync(string investorWallet)
        {
            var refundFunction = new RefundFunction();
                refundFunction.InvestorWallet = investorWallet;
            
             return ContractHandler.SendRequestAsync(refundFunction);
        }

        public Task<TransactionReceipt> RefundRequestAndWaitForReceiptAsync(string investorWallet, CancellationTokenSource cancellationToken = null)
        {
            var refundFunction = new RefundFunction();
                refundFunction.InvestorWallet = investorWallet;

...
{
    public class ProjectStageUnderReviewValidator : AbstractValidator<Stage>
    {
        public ProjectStageUnderReviewValidator()
        {
            ...
            // RuleFor(s => s.StartDate).NotNull().Must((offset) => offset > DateTimeOffset.UtcNow.Date.AddDays(Constants.MinimumStartDateThresholdInDays)).WithErrorCode($"VALIDATION_INVALID_STAGE_START_DATE");

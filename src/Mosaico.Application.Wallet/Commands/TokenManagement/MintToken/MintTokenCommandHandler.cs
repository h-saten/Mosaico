...
{
    public class MintTokenCommandHandler : IRequestHandler<MintTokenCommand>
    {
        ...
        public MintTokenCommandHandler(IWalletDbContext walletDbContext, ICurrentUserContext currentUser, IDaoService daoService, IBusinessManagementClient businessManagement, IWalletDispatcher walletDispatcher, IBackgroundJobProvider backgroundJob, ILogger logger = null)
        {
        ...
        public async Task<Unit> Handle(MintTokenCommand request, CancellationToken cancellationToken)
        {
            ...
            _backgroundJob.Execute(() => MintTokenAsync(token.Id, _currentUser.UserId, request.Amount));
            
            return Unit.Value;
        }

        [AutomaticRetry(Attempts = 0, OnAttemptsExceeded = AttemptsExceededAction.Delete)]
        public async Task MintTokenAsync(Guid tokenId, string userId, decimal amount)
        {
            ...
            if (token.Status == TokenStatus.Deployed)
            {
                ...
                try
                {
                    await _daoService.MintTokenAsync(token.Network, new Daov1Configurations.MintTokenConfiguration
                    {
                        Amount = amount,
                        Decimals = 18,
                ...
                {
                    await _walletDispatcher.TokenMintingFailed(userId, ex.Message);
                    return;
                }
            }
            token.TotalSupply += amount;
            await _walletDbContext.SaveChangesAsync();
            await _walletDispatcher.TokenMinted(userId, token.Id);
        }
    }
}
...
{
    [EventInfo(nameof(SendResetPasswordEmailOnUserRestore),  "users:id")]
    [EventTypeFilter(typeof(UserRestoredEvent))]
    public class SendResetPasswordEmailOnUserRestore : EventHandlerBase
    {
        ...
        public SendResetPasswordEmailOnUserRestore(IIdentityEmailService emailService, UserManager<ApplicationUser> userManager, IIndex<string, string> urls, ILogger logger = null)
        {
        ...
        {
            var userEvent = @event?.GetData<UserRestoredEvent>();
            if (userEvent != null)
            {
